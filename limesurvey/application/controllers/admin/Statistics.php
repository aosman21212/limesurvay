...
class Statistics extends SurveyCommonAction
{
    ...
    public function setSurveySubmittedStatus()
{
    $submittedStatus = Yii::app()->request->getPost('survey_submitted_status');

    // Define the allowed values for survey_submitted_status
    $allowedValues = array('Yes', 'No', 'Partial');

    if (in_array($submittedStatus, $allowedValues)) {
        Yii::app()->session['survey_submitted_status'] = $submittedStatus;
    }
}
