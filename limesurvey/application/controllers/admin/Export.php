...
class Export extends SurveyCommonAction
{
    ...
    public function exportresults()
    {
        ...
        if (Yii::app()->request->getPost('response_id')) {
            ...
            $startDate = Yii::app()->request->getPost('start_date');
            $endDate = Yii::app()->request->getPost('end_date');
            $startSurveySentDate = Yii::app()->request->getPost('start_survey_sent_date');
            $endSurveySentDate = Yii::app()->request->getPost('end_survey_sent_date');
            $startChatStartDate = Yii::app()->request->getPost('start_chat_start_date');
            $endChatStartDate = Yii::app()->request->getPost('end_chat_start_date');
            $selectedChatDate = Yii::app()->request->getPost('selected_chat_date');
            $whatsappNumber = Yii::app()->request->getPost('whatsapp_number');
            $servedBy = Yii::app()->request->getPost('served_by');
            $submittedStatus = Yii::app()->request->getPost('survey_submitted_status');
        
            // Add date range filter based on submitdate field (assuming 'submitdate' is a datetime field)
            if ($startDate && $endDate) {
                $sSubmitDateFilter = "{{survey_{$iSurveyID}}}.submitdate BETWEEN '{$startDate}' AND '{$endDate}'";
            } else {
                $sSubmitDateFilter = '';
            }
        
            // Add survey sent date range filter (assuming 'survey_sent_date' is a date field)
            if ($startSurveySentDate && $endSurveySentDate) {
                $sSurveySentDateFilter = "{{survey_{$iSurveyID}}}.survey_sent_date BETWEEN '{$startSurveySentDate}' AND '{$endSurveySentDate}'";
            } else {
                $sSurveySentDateFilter = '';
            }
            //   // Add survey sent date range filter (assuming 'survey_sent_date' is a date field)
            //   if ($startChatStartDate && $endChatStartDate) {
            //     $sChatStartDateFilter = "({{survey_{$iSurveyID}}}.chat_start_date BETWEEN '{$startChatStartDate}' AND '{$endChatStartDate}')";
            // } else {
            //     $sChatStartDateFilter = "({{survey_{$iSurveyID}}}.chat_start_date BETWEEN '{$startChatStartDate}' AND '{$endChatStartDate}')";
            // }
            // // Check if both start and end chat start datetime values are provided
  
        
           // Check if both $selectedStartDate and $selectedEndDate are set
if ($selectedStartDate && $selectedEndDate) {
    // Convert $selectedStartDate and $selectedEndDate to 'Y-m-d' formatted date strings
    $startDate = date('Y-m-d', strtotime($selectedStartDate));
    $endDate = date('Y-m-d', strtotime($selectedEndDate));

    // Create a SQL filter condition for chat_start_date
    // It checks if the date portion of chat_start_date falls within the selected date range
    $sChatStartDateFilter = "DATE({{survey_{$iSurveyID}}}.chat_start_date) BETWEEN '{$startDate}' AND '{$endDate}'";
} else {
    // If either $selectedStartDate or $selectedEndDate is not set, no date filter is applied
    $sChatStartDateFilter = '';
}

        
            // Add "Served By" filter (assuming 'Served_By' is a varchar field)
            if ($servedBy) {
                $sServedByFilter = "{{survey_{$iSurveyID}}}.Served_By = '{$servedBy}'";
            } else {
                $sServedByFilter = '';
            }
        
            // Add "WhatsApp Number" filter (assuming 'whatsapp_number' is a varchar field)
            if ($whatsappNumber) {
                $sWhatsAppNumberFilter = "{{survey_{$iSurveyID}}}.whatsapp_number = '{$whatsappNumber}'";
            } else {
                $sWhatsAppNumberFilter = '';
            }
        
            // Add "Submitted Status" filter (assuming 'survey_submitted_status' is an enum field)
            if ($submittedStatus) {
                $sSubmittedStatusFilter = "{{survey_{$iSurveyID}}}.survey_submitted_status = '{$submittedStatus}'";
            } else {
                $sSubmittedStatusFilter = '';
            }
        
            // Combine all filters
            $combinedFilters = array_filter([
                $sSubmitDateFilter,
                $sSurveySentDateFilter,
                $sServedByFilter,
                $sWhatsAppNumberFilter,
            ...
            if (!empty($combinedFilters)) {
                $sFilter = implode(' AND ', $combinedFilters);
            } else {
                $sFilter = '';
            }
        }
